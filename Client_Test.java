...
public class Client_Test {
	...
	public static void main(String[] args) {
		SparkSession spark = SparkSession.builder().appName("CustomerETL").master("local[*]").getOrCreate();

		List<Customer> customerList = getCustomerData();
		Dataset<Row> customerDF = spark.createDataFrame(customerList, Customer.class);
		customerDF.createOrReplaceTempView("customers");
		Dataset<Row> transformedData = spark.sql(
				"SELECT custNo, UPPER(firstname) AS firstname,lastname,gender, age,profession,contactNO,emailId,city, state,isActive FROM customers");
